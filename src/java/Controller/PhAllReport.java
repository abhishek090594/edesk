 ...
 * @author admin
 */
public class PhAllReport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
         ...
         */         HttpSession sobj = request.getSession(false);
           ...
           try{
                ...
                ResultSet resultSet = statement.executeQuery("SELECT count( * ) FROM staff AS s, panelhead AS ph, panelmembers AS pm, studentinquiry AS si WHERE ph.ph_id = pm.ph_id AND s.stf_id = pm.stf_id AND ph.ph_id ="+ph_id+" AND pm.stf_id = si.stf_id AND si.stud_status =1 ");
               // phRs1 = statement.executeQuery("SELECT count( * ) FROM staff AS s, panelhead AS ph, panelmembers AS pm, studentinquiry AS si WHERE ph.ph_id = pm.ph_id AND s.stf_id = pm.stf_id AND ph.ph_id ="+ph_id+" AND pm.stf_id = si.stf_id AND si.stud_status =1 ");
		...
		//JDBCPieDataset dataset = new JDBCPieDataset(connect);
                ...
                while(resultSet.next())
                {
                  //  int stf_id=resultSet.getInt("stf_id");
                    ...
                    resultSet = statement.executeQuery("SELECT count( * ) FROM staff AS s, panelhead AS ph, panelmembers AS pm, studentinquiry AS si WHERE ph.ph_id = pm.ph_id AND s.stf_id = pm.stf_id AND ph.ph_id ="+ph_id+" AND pm.stf_id = si.stf_id AND si.stud_status =0 " );
                    while (resultSet.next()) {
