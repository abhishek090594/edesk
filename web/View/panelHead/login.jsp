...
<html>
<head>
	<title>E-Registration Desk - Panel Head</title>
		<meta charset="utf-8">
...
<body>
	<div class="main">
		<div class="login-form">
			<h1>Panel Head Login</h1>
        <%
                           HttpSession sobj = request.getSession(false);
                            if(sobj != null)
                            {
                                if(sobj.getAttribute("ph_is_logged_in") != null)
                                {
                                   response.sendRedirect(request.getContextPath()+"/phViewDashboardController");
                                }
                            }
			...
			<a href="#" class="close" onclick="return closediv()">close</a>
					...
					--></div>
                        <form method="post" action="<%= request.getContextPath() %>/phCheckLoginController">
